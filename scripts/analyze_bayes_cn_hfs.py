...
def main(source, datadir="alma_spectra/", resultsdir="results/"):
    ...
    # Plot CNModel BICs
    n_clouds = result["opt_results"].keys()
    ...
    fig.savefig(os.path.join(outdir, f"{source}_CN_data.pdf"), bbox_inches="tight")
    plt.close(fig)
            ...
            for sample in trace.sample[::thin]:
                ...
                for key in posterior_predictive.keys():
                    posterior_predictive[key].append(
                        model.model[key].eval(sim_params, on_unused_input="ignore")
